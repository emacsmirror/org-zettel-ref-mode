...
3. 选择自己希望的标记风格
或者，对于鼠标操作，选中文本后右键点击，从“Org Zettel Ref Highlight”上下文菜单中选择所需的高亮类型。
...
* 版本历史
- v0.6.3 (2025-12-05)
  - 新增：完整支持通过上下文菜单（鼠标右键）添加高亮。用户现在可以右键点击应用高亮，并解决了之前出现的文本视觉重复和错乱问题。现在，无论是直接命令还是通过上下文菜单操作，高亮功能均能正确地将选定文本替换为标记，并稳定显示视觉效果。
- v0.6.2 (2025-12-02)
  - 修复：`org-zettel-ref-list.el` 中重复的函数定义导致重命名提示（标题和关键词）丢失自动补全功能的问题。
- v0.6.1 (2025-07-28)
  - 新增：高亮支持隐藏标记，仅对标注文本上色，标记前后缀隐藏但锚点仍可跳转，减少视觉干扰。
  - 新增：单文件模式下，直接打开统一概览文件也会自动激活 org-zettel-ref 的 overlay 与局部变量，无需先打开源文件。
  - 新增：在 minibuffer 选择高亮类型时可预览颜色和前缀（annotation + affixation 适配常见补全 UI）。
  - 新增：如果注释/概览文件已打开，`org-zettel-ref-init` 复用现有缓冲区，不再重复打开。
  - 新增：打开面板时会分屏或复用窗口，不再独占整个 frame。
  - 新增：当注释/概览缓冲区已存在但未显示时，`org-zettel-ref-init` 会自动展示它。
  - 新增：minibuffer 高亮类型预览启用文本属性渲染。
  - 新增：面板显示增加兜底逻辑，保证能看到面板（复用、弹出，或最后在当前窗口显示）。
  - 新增：高亮类型补全提供注释式预览（前缀 + 着色示例）。
  - 新增：在源文件高亮处按 `C-c C-o` 也能跳到概览对应条目，即便标记被隐藏。
- v0.6.0 (2025-07-27)
  - 新增：高亮显示格式选项
